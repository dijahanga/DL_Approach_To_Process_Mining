Copyright (c) 2020 Khadijah Hanga
...
# -*- coding: utf-8 -*-
"""Model1(Unidirectional)

This script trains a one-layer LSTM model on one of the data files in the data folder of this repository.
...
#model.add(Embedding(MAX_NB_WORDS, EMBEDDING_DIM, input_length=X.shape[1]))#without splitting to train/test
model.add(LSTM(100, dropout=0.2, recurrent_dropout=0.2))
model.add(Dense(Y_train.shape[1], activation='softmax'))
#model.add(Dense(Y.shape[1], activation='softmax'))#without splitting to train/test
...
model = load_model('Unilstm_model.h5')#the model can be loaded again and used (from a different script in a different Python session)
